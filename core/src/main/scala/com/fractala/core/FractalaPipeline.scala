...
/** The main entry point for the Fractala library. * This object orchestrates the entire fractal generation pipeline by
  * combining the parsing, grammar construction, and iteration stages. It abstracts away the internal domain complexity,
  * providing a clean API for client applications.
  */
object FractalaPipeline {
  ...
  /** Processes the provided L-System DSL code and generates a lazy stream of drawing instructions.
    ...
    *   1. Parses the raw string into a valid L-System model (`Config`, `Axiom`, `Rules`). 2. Constructs a
    *      [[StochasticGrammar]] and registers the parsed production rules. 3. Initializes a
    *      [[RecursiveLSystemIterator]] to evaluate the axiom over the specified generations.
    *
    * @param dslCode
